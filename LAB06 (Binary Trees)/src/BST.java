...
//Exercise 1
	public int count(BSTNode root) {
		...
		return count(root.left) + count(root.right) + 1; // call the count recursively to count children then add 1 for
															// the root
	}
...
//Exercise 2
	public boolean isLeaf(T value) {
		...
		if (p != null) {
			...
			while (!queue.isEmpty()) {
				p = queue.dequeue();
				if (p.left == null && p.right == null && p.el == value)
					return true;
				if (p.left != null)
					queue.enqueue(p.left);
				if (p.right != null)
...
//Exercise 3
	public int countLeaves(BSTNode root) {
		...
		if ((root.left == null) && (root.right == null)) { // when it reaches a leaf it counts 1
			return 1;
		}
		return countLeaves(root.left) + count(root.right); // calls back the method recursively to sum all leafs
															// together
	}
...
//Exercise 4
	public int height(BSTNode root) {
		...
		if (root.left == null && root.right == null) { // if it reaches a leaf it counts 1
			return 1;
		}

		// take the maximum of either right or left subtree (recursively)
		int STH = Math.max(height(root.left), height(root.right));

		int height = STH + 1; // adds 1 to the max subtree height to count the root
		return height;
	}

//Exercise 5: (I created them myself since I didn't know if we were supposed to create the methods or use the given)

	// preorder traversal
	public void predft(BSTNode root) {
		if (root == null)
			return;
	...
	// inorder traversal
	public void indft(BSTNode root) {
		if (root == null)
			return;
		indft(root.left);
		System.out.print(root.el + " ");
	...
	// postorder traversal
	public void postdft(BSTNode root) {
		if (root == null)
			return;
		postdft(root.left);
		postdft(root.right);
